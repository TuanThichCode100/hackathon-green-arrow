...
const ETHNICS = ['Kinh', 'Thái', 'Mông', 'Khơ Mú', 'Dao', 'Tày', 'Mường'];
const LANGUAGES = [{ value: 'vi', label: 'Tiếng Kinh' }, { value: 'hmn', label: 'Tiếng Mông' }, { value: 'tai', label: 'Tiếng Thái' }, { value: 'khmu', label: 'Tiếng Khơ Mú' }, { value: 'dao', label: 'Tiếng Dao' }, { value: 'tay', label: 'Tiếng Tày' }, { value: 'muong', label: 'Tiếng Mường' }];
const languageForEthnic: Record<string, string> = { Kinh: 'vi', 'Thái': 'tai', 'Mông': 'hmn', 'Khơ Mú': 'khmu', Dao: 'dao', 'Tày': 'tay', 'Mường': 'muong' };
...
type Resident = { id: number; commune_id: number; name: string; phone: string; ethnic: string; primary_language: string; literate: boolean };
...
export default function ResidentsDB({ isProv, assignedCommuneId, showToast, communesData }: { isProv?: boolean; assignedCommuneId?: number | string | null; showToast: (message: string, tone?: 'success' | 'warning' | 'error') => void; communesData: Commune[] }) {
  ...
  const [formData, setFormData] = useState({ name: '', phone: '', ethnic: 'Kinh', primary_language: 'vi', literate: true });
  ...
  const openModal = (resident: Resident | null = null) => {
    ...
    setFormData(resident ? { name: resident.name, phone: resident.phone, ethnic: resident.ethnic, primary_language: resident.primary_language || languageForEthnic[resident.ethnic] || 'vi', literate: resident.literate } : { name: '', phone: '', ethnic: 'Kinh', primary_language: 'vi', literate: true });
    setFormError('');
    setIsModalOpen(true);
  };
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const languageIndex = indexOf('ngonngusudungchinh', 'ngonngu', 'primarylanguage');
    ...
    const records = rows.slice(1).map((row, index) => ({
      ...
      primary_language: languageIndex < 0 ? undefined : row[languageIndex] ?? undefined,
