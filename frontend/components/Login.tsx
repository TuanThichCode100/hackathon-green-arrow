...
export default function Login({ onLoginSuccess, onCancel }: { onLoginSuccess: (user: User) => void; onCancel?: () => void }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!isEmail && !isPhone) {
      return setError('Vui lòng nhập đúng email hoặc số điện thoại (10 chữ số).');
    }
    ...
    try {
      ...
      const loginEmail = isEmail ? username : `${username}@dienbien.gov.vn`;
      ...
      const nextUser: User = {
        ...
        role: data.user.user_metadata?.role === 'xa' ? 'xa' : 'tinh',
        commune_id: data.user.user_metadata?.commune_id,
      };
  ...
  return (
    <main className="auth-page">
      <section className="auth-context">
        <div className="brand-mark"><TreeStructure size={21} weight="bold" /></div>
        <p className="eyebrow">GreenForecast · Điện Biên</p>
        <h1>Thông tin đúng địa bàn, hành động đúng thời điểm.</h1>
        <p>Không gian tác nghiệp dành cho cán bộ theo dõi rủi ro, kiểm tra mức tiếp cận và điều phối cảnh báo.</p>
      </section>
      <section className="auth-form-wrap">
        <form className="auth-form" onSubmit={submit}>
          ...
          <label><span>Số điện thoại / Email</span><input value={username} onChange={(event) => setUsername(event.target.value)} inputMode="text" autoComplete="username" placeholder="09xx xxx xxx hoặc email" /></label>
