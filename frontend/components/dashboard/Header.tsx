...
const titles: Record<string, string> = { map: 'Bản đồ rủi ro', overview: 'Tổng quan vận hành', communes: 'Địa bàn và mức tiếp cận', policy: 'Văn bản chỉ đạo', channels: 'Phân phối cảnh báo', roles: 'Quyền truy cập', database: 'Dữ liệu dân cư', feedback: 'Phản ánh thông tin' };
const ranges = [['today', 'Hôm nay'], ['24h', '24 giờ'], ['7d', '7 ngày'], ['30d', '30 ngày']];
type Notice = { id: number; source?: 'document' | 'feedback'; actor_name: string; actor_role: string; title: string; subtitle: string; created_at: string; read: boolean; request_id?: number; actionable?: boolean };
...
export default function Header({ view, timeRange, setTimeRange, clock, user }: { view: string; timeRange: string; setTimeRange: (value: string) => void; clock: string; user: User | null }) {
  ...
  const load = async (): Promise<Notice[]> => {
    ...
    setLoading(true);
    try {
      const [documentResponse, feedbackResponse] = await Promise.all([
        fetch(`${API_BASE_URL}/api/documents/notifications`, { headers }),
        fetch(`${API_BASE_URL}/api/feedback/notifications`, { headers }),
      ]);
      const documentBody = await documentResponse.json();
      const feedbackBody = await feedbackResponse.json();
      const notices = [
        ...(documentResponse.ok ? (documentBody.data || []).map((item: Notice) => ({ ...item, source: 'document' as const })) : []),
        ...(feedbackResponse.ok ? (feedbackBody.data || []) : []),
      ].sort((a: Notice, b: Notice) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      setItems(notices); return notices;
    } catch { setItems([]); return []; } finally { setLoading(false); }
  };
  ...
  const toggleNotifications = async () => {
    ...
    const documentIds = latest.filter((item) => !item.read && item.source !== 'feedback').map((item) => item.id);
    const feedbackIds = latest.filter((item) => !item.read && item.source === 'feedback').map((item) => item.id);
    if (documentIds.length || feedbackIds.length) {
      await Promise.all([
        documentIds.length ? fetch(`${API_BASE_URL}/api/documents/notifications/read`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...headers }, body: JSON.stringify({ event_ids: documentIds }) }) : Promise.resolve(),
        feedbackIds.length ? fetch(`${API_BASE_URL}/api/feedback/notifications/read`, { method: 'POST', headers: { 'Content-Type': 'application/json', ...headers }, body: JSON.stringify({ feedback_ids: feedbackIds }) }) : Promise.resolve(),
      ]);
