...
import { CheckCircle, Spinner, X } from '@phosphor-icons/react';
...
export default function DocumentReviewModal({ documentId, onClose, onDone, communes }: { documentId: number; onClose: () => void; onDone: () => void; communes: Array<{ id: number; name: string }> }) {
  ...
  useEffect(() => {
    ...
    const loadPreview = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/documents/${documentId}/preview`, { headers: token ? { Authorization: `Bearer ${token}` } : {} });
        const body = await res.json();
        if (res.status === 202) {
          if (!cancelled) {
            setReviewStatus('processing');
            timer = setTimeout(loadPreview, 2000);
          }
          return;
        }
        if (!res.ok) throw new Error(body.detail || 'Không thể tạo bản xem trước');
        if (!cancelled) {
          setDraft({ ...emptyDraft, ...body.data.draft });
          setEvidence(body.data.evidence || {});
  ...
  const save = async (approve = false) => {
    ...
    try {
      ...
      const body = await res.json();
      if (!res.ok) throw new Error(body.detail || 'Không thể lưu bản xem trước');
      if (approve) onDone(); else setEvidence(body.data.evidence || evidence);
    } catch (err) { setError(err instanceof Error ? err.message : 'Không thể lưu'); }
    finally { setSaving(false); }
  };
  const cancel = async () => {
    if (!confirm('Thoát và không lưu bản nháp này?')) return;
    await fetch(`${API_BASE_URL}/api/documents/${documentId}/cancel`, { method: 'POST', headers: token ? { Authorization: `Bearer ${token}` } : {} });
    onDone();
  };
  ...
  return <div className="review-backdrop" role="presentation"><section className="document-review" role="dialog" aria-modal="true" aria-labelledby="review-title"><header className="document-review-header"><div><p className="eyebrow">Bản nháp cần xác nhận</p><h2 id="review-title">Kiểm tra thông tin văn bản</h2></div><button className="icon-button" onClick={cancel} aria-label="Thoát và không lưu"><X size={18} /></button></header><div className="document-review-body">
    ...
    {reviewStatus === 'ready' && <><div className="review-grid">
      ...
      <label className="review-wide">Tóm tắt chỉ đạo<textarea value={draft.llm_summary || ''} placeholder="Cần cấu hình SLM để tạo tóm tắt tự động; bạn có thể bổ sung thủ công." onChange={(e) => set('llm_summary', e.target.value)} /></label>
      <label className="review-wide">Hành động yêu cầu<textarea value={draft.required_actions || ''} placeholder="Cần cấu hình SLM để gợi ý hành động; bạn có thể bổ sung thủ công." onChange={(e) => set('required_actions', e.target.value)} /></label>
    </div><fieldset className="scope-picker"><legend>Địa bàn áp dụng</legend><label><input type="radio" checked={draft.scope_type === 'province'} onChange={() => set('scope_type', 'province')} /> Toàn tỉnh</label><label><input type="radio" checked={draft.scope_type === 'communes'} onChange={() => set('scope_type', 'communes')} /> Các xã/phường cụ thể</label>{draft.scope_type === 'communes' && <div className="commune-checks">{communes.map((commune) => <label key={commune.id}><input type="checkbox" checked={draft.commune_ids.includes(commune.id)} onChange={() => set('commune_ids', draft.commune_ids.includes(commune.id) ? draft.commune_ids.filter((id: number) => id !== commune.id) : [...draft.commune_ids, commune.id])} /> {commune.name}</label>)}</div>}</fieldset><label className="original-toggle"><input type="checkbox" checked={draft.show_original_to_province} onChange={(e) => set('show_original_to_province', e.target.checked)} /> Hiển thị bản gốc của tài liệu cho cán bộ tỉnh</label></>}
  </div>{reviewStatus === 'ready' && <footer className="document-review-footer"><button className="secondary-button" disabled={saving} onClick={() => save(false)}>Lưu bản nháp</button><button className="primary-button" disabled={saving} onClick={() => save(true)}><CheckCircle size={17} />{saving ? 'Đang lưu…' : 'Xác nhận văn bản'}</button></footer>}</section></div>;
}
